...
type RecipeLite = {
  id: number;
  title: string;
  tags?: string[];              // optional: we’ll handle if missing
  _match?: number;              // computed match % for this pantry (0..1)
};
...
async function fetchSavedRecipes(): Promise<RecipeLite[]> {
  // Prefer server filtering if available:
  const candidates: string[] = [
    "/api/recipes?saved=true",  // preferred if implemented
    "/api/recipes",             // fallback (we’ll filter on client if `saved` flags exist)
  ];
  ...
  for (const url of candidates) {
    try {
      ...
      // normalize to RecipeLite (best-effort: tolerate missing fields)
...
export default function SavedRecipesPage({
  ...
  const [tags, setTags] = useState<string[]>([]); // tag filters
  const [sort, setSort] = useState<"az" | "za" | "match">("match");
  const [loadingMatches, setLoadingMatches] = useState(false);
  const [matched, setMatched] = useState<Record<number, number>>({}); // id -> coverage 0..1

  const inputRef = useRef<HTMLInputElement>(null);

  // Compute list with match % (computed on demand or after load)
  useEffect(() => {
  ...
  const filtered = useMemo(() => {
    ...
    const sorter = {
      az: (a: RecipeLite, b: RecipeLite) => a.title.localeCompare(b.title),
      za: (a: RecipeLite, b: RecipeLite) => b.title.localeCompare(a.title),
      match: (a: RecipeLite, b: RecipeLite) => (b._match ?? 0) - (a._match ?? 0) || a.title.localeCompare(b.title),
    }[sort];

    return [...byTags].sort(sorter);
              ...
              <div className="flex flex-wrap gap-2">
                {tags.map((t, i) => (
                  <span key={`${t}-${i}`} className="inline-flex items-center gap-1 rounded-full bg-gray-100 px-2 py-1 text-xs">
                    {t}
                    <button onClick={() => removeTag(i)} className="rounded-full px-1 text-gray-500 hover:bg-gray-200" aria-label={`Remove ${t}`}>×</button>
                  </span>
                ))}
                <input
                  ref={inputRef}
              ...
              {r.tags?.length ? (
                <div className="flex flex-wrap gap-1">
                  {r.tags.slice(0, 6).map((t, i) => (
                    <span key={`${r.id}-t-${i}`} className="rounded-full bg-gray-100 px-2 py-0.5 text-[11px]">{t}</span>
                  ))}
                  {r.tags.length > 6 && (
