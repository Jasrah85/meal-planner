...
type RecipeListRow = {
  ...
  // OPTIONAL (if your API returns it; used to show current assignment)
  pantryId?: number | null;
  pantryName?: string | null;
};

/* ----------------------------- Pantries types ---------------------------- */
type Pantry = { id: number; name: string };
...
/** Browse: if q is empty, ask backend for a random/mixed feed */
async function fetchCombinedBrowse(
  ...
  else params.set("random", "1");
  params.set("page", String(page));
  params.set("pageSize", String(pageSize));
  const url = `/api/recipes/browse?${params.toString()}`;
  const r = await fetch(url, { cache: "no-store" });
  if (!r.ok) throw new Error("Browse failed");
  return (await r.json()) as CombinedBrowseResponse;
}

/** Find a local recipe id by its ext tag (for remove) */
...
/** Library querying: title and ingredient search + tag filter */
async function fetchRecipes(q: string, tag: string) {
  const params = new URLSearchParams();
  if (q) params.set("query", q); // recommend: backend searches title + ingredient names
...
/** Pantries list for assignment dropdown */
async function fetchPantries(): Promise<Pantry[]> {
  const r = await fetch(`/api/pantries`, { cache: "no-store" });
  if (!r.ok) return [];
  const json = (await r.json()) as { pantries: Pantry[] } | Pantry[];
  return Array.isArray(json) ? json : json.pantries;
}
...
export default function RecipesPage() {
  ...
  const sortedLibrary = useMemo(() => {
    ...
    switch (sort) {
      case "newest":
        // assuming higher id = newer; swap if you have createdAt
        copy.sort((a, b) => b.id - a.id);
        break;
      case "ingredients_desc":
        copy.sort(
          (a, b) => (b._count?.ingredients ?? 0) - (a._count?.ingredients ?? 0)
        );
        break;
      case "title_asc":
      default:
  ...
  /* ---------- Pantry assignment ---------- */
  const { data: pantries } = useQuery({
    queryKey: ["pantries"],
    queryFn: fetchPantries,
  });

  const assignPantry = useMutation({
    mutationFn: async ({ recipeId, pantryId }: { recipeId: number; pantryId: number | null }) => {
      // prefer a dedicated endpoint; fall back to PATCH recipe if you already have it
      const res = await fetch(`/api/recipes/assign-pantry`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ recipeId, pantryId }),
      });
      if (!res.ok) throw new Error("Failed to assign pantry");
      return res.json();
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["recipes"] }),
  });
  ...
  const [browseQ, setBrowseQ] = useState(""); // empty initially => random feed
  ...
  const pageSize = 12;
    ...
    enabled: true, // show random feed on first load
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        {/* Library list with pantry assignment + remove */}
        {libLoading ? (
          <div>Loading…</div>
        ) : (
              ...
              <li key={r.id} className="p-3 flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <div className="flex items-center gap-1">
                    <label htmlFor={`pantry-${r.id}`} className="text-xs text-gray-500">
                      Pantry
                    </label>
                    <select
                      id={`pantry-${r.id}`}
                      ...
                      onChange={(e) =>
                        assignPantry.mutate({
                          recipeId: r.id,
                          pantryId: e.target.value ? Number(e.target.value) : null,
                        })
                      }
                    >
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {!browseQ && (
            <span className="text-xs text-gray-500">
              Showing a mixed/random feed. Use search to filter.
            </span>
          )}
        </div>
